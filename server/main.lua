			...
			for k,v in ipairs(vehicles) do
				for k2,v2 in ipairs(categories) do
					if v2.name == v.category then
						...
						vehicles[k].categoryLabel_sc = v2.label_sc
						vehicles[k].categoryLabel_tc = v2.label_tc
						break
					end
				end
			end
			...
			table.insert(tasks, function(cb)
				...
				if xPlayer then
					if xPlayer.getAccount('bank').money >= v.rent_price then
						...
						xPlayer.showNotificationAndTranslate({'paid_rental', ESX.Math.GroupDigits(v.rent_price), v.plate})
					else
						xPlayer.showNotificationAndTranslate({'paid_rental_evicted', ESX.Math.GroupDigits(v.rent_price), v.plate})
