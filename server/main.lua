...
TriggerClientEvent('esx_vehicleshop:forServer', 'esx_phone:registerNumber', 'cardealer', 'dealer_customers', false, false)
TriggerClientEvent('esx_vehicleshop:forServer', 'esx_society:registerSociety', 'cardealer', 'car_dealer', 'society_cardealer', 'society_cardealer', 'society_cardealer', {type = 'private'})
...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function (vehicleProps)
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		...
		TriggerClientEvent('esx:showVehicleshopNotification', _source, 'vehicle_belongs', vehicleProps.plate)
	end)
end)
...
AddEventHandler('esx_vehicleshop:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			TriggerClientEvent('esx:showVehicleshopNotification', _source, 'vehicle_sold_for', vehicleModel, ESX.Math.GroupDigits(price))
		else
	...
	MySQL.Async.fetchAll('SELECT * FROM rented_vehicles', {}, function (result)
		for i=1, #result, 1 do
			...
			if xPlayer ~= nil then
				...
				TriggerClientEvent('esx:showVehicleshopNotification', xPlayer.source, 'paid_rental', ESX.Math.GroupDigits(result[i].rent_price))
